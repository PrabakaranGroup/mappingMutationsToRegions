...
vcfMutation                 = pd.read_csv(io.BytesIO(s3.get_object(Bucket=bucket, Key='prabakaranLab/cosmic/coding/CosmicCodingMuts.vcf-14-12-17-SP')['Body'].read()),delimiter='\t', dtype='unicode',skiprows=13)
...
sORFs = pd.read_csv(io.BytesIO(s3.get_object(Bucket=bucket, Key='prabakaranLab/arka_replication/Altorf.csv')['Body'].read()),delimiter=',', dtype='unicode')
...
def mappingMutation(mut,sORfsChrom, mutationChrom ):
    ...
    for orf in range(len(sORfsChrom.ChrN)):
        #print(sORfsChrom)
        with open('/home/ngs/narumeena/Documents/arkarplication/altorfs/coding/CosmicCodingVariantsMappedToHumanAltORFs.csv', 'a') as gh:
            writer = csv.writer(gh)
	    #print(sORfsChrom.Chromosome[orf])
	    #print(mutationChrom['#CHROM'][mut])
            if(mutationChrom['#CHROM'][mut]==sORfsChrom.ChrN[orf]):
                #print("chrome match")
		#print(float( sORfsChrom.Start_position[orf]))
		#print(float( mutationChrom.POS[mut]))
                #print(float( sORfsChrom.End_position[orf]))
                if(float( sORfsChrom.Start[orf]) <= float( mutationChrom.POS[mut]) <= float( sORfsChrom.Stop[orf])):
...
for chrom in chromList:
    ...
    sORfsChrom      = sORFs.loc[sORFs['ChrN'] == chrom]
